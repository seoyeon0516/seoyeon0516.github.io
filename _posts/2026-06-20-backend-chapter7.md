...
1. DB에서 데이터를 가져올 때는 리파지터리를 이용한다. 따라서 `articleRepository`의 `findById(id)` 메서드로 데이터를 찾아 가져온다. 만약 데이털르 찾지 못하면 `null`을 반환하고 데이터를 찾았다면 `Article` 타입의 `articleEntity`로 저장한다.

2. 변수 `id`는 메서드의 매개변수로 받아 오고 자료형은 `Long`으로 작성한다. `GetMapping()` 어노테이션의 URL 주소에 있는 id를 받아오는 것이므로 데이터 타입 앞에 `@PathVariable` 어노테이션을 추가한다.
